...
def guardar_archivo(uploaded_file, carpeta, centro, folio, nombre_hoja):
    ...
    destino_dir = ARCHIVOS_DIR / carpeta_limpia / centro_limpio
...
def insertar_documento(carpeta, centro, nombre_hoja, folio, fecha_documento, observaciones, palabras_clave, archivo_path):
    con = conectar()
    cur = con.cursor()
    cur.execute("""
        INSERT INTO documentos (
            fecha_captura, carpeta_principal, centro, nombre_hoja, folio,
            fecha_documento, observaciones, palabras_clave, archivo_path, usuario
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (
...
def eliminar_documento(id_doc):
    con = conectar()
    cur = con.cursor()
    cur.execute("SELECT archivo_path FROM documentos WHERE id = ?", (int(id_doc),))
    row = cur.fetchone()
...
if menu == "🏠 Inicio":
    ...
    if st.button("💾 Guardar digitalización"):
        if not nombre_hoja.strip():
            ...
            ruta = guardar_archivo(archivo, carpeta, centro, folio, nombre_hoja)
            insertar_documento(carpeta, centro, nombre_hoja, folio, fecha_documento, observaciones, palabras_clave, ruta)
            st.success("Documento guardado correctamente.")
            st.info(f"Ruta guardada: {ruta}")
            st.rerun()
    ...
    if resultado.empty:
        ...
        data = leer_archivo(doc["archivo_path"])
        if data:
            extension = Path(doc["archivo_path"]).suffix.lower()
            mime = "application/pdf" if extension == ".pdf" else "image/png"
            st.download_button(
                "📄 Descargar / abrir documento",
                data=data,
                file_name=os.path.basename(doc["archivo_path"]),
                mime=mime
            )
        else:
            st.error("El archivo no se encontró en la carpeta local.")
    ...
    if st.session_state.rol_pc != "Administrador":
        ...
        if st.button("🗑️ Eliminar documento"):
            eliminar_documento(id_eliminar)
