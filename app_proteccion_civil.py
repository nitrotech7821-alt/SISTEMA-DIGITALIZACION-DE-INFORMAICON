...
if not st.session_state.logueado_pc:
    mostrar_login()
    st.stop()
...
def leer_documentos_firebase_cache():
    if not firebase_disponible():
        return pd.DataFrame()

    try:
        # IMPORTANTE: limit y timeout para evitar que Streamlit se quede pensando.
        docs = db_firebase.collection(COLECCION_FIRESTORE).limit(300).stream(timeout=10)
        registros = []
        for doc in docs:
            d = doc.to_dict()
...
def leer_documentos():
    df_fb = leer_documentos_firebase_cache()
    if not df_fb.empty:
        return df_fb
...
def eliminar_documento(id_doc, firebase_id="", archivo_path="", archivo_storage_path=""):
    if firebase_disponible() and firebase_id:
        try:
            db_firebase.collection(COLECCION_FIRESTORE).document(firebase_id).delete(timeout=10)
        except Exception:
            pass
...
if st.sidebar.button("🔄 Cargar datos de Firebase"):
    leer_documentos_firebase_cache.clear()
    df_nube = leer_documentos_firebase_cache()
    if not df_nube.empty:
        st.session_state["df_nube"] = df_nube
    st.rerun()
...
# MODO RAPIDO:
# Para evitar que la app se quede pensando, NO lee Firestore automáticamente al abrir.
# Guarda en Firebase normalmente, pero muestra primero la base local.
if "df_nube" in st.session_state and not st.session_state["df_nube"].empty:
    df = st.session_state["df_nube"]
else:
    df = leer_documentos_local()
